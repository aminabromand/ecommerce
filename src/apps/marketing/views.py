...
class MarketingPreferenceUpdateView(SuccessMessageMixin, UpdateView):
	...
	def dispatch(self, *args, **kwargs):
		...
		if not user.is_authenticated():
			return redirect('/login/?next=/settings/email/') # return HttpResponse('Not allowed', status= 400)
...
class MailchimpWebhookView(CsrfExemptMixin, View):
	...
	def post(self, request, *args, **kwargs):
		...
		if str(list_id) == str(MAILCHIMP_EMAIL_LIST_ID):
			...
			response_status, response = Mailchimp().check_subscribtion_status(email)
			sub_status = resonse.get('status')
